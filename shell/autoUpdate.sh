...
# port=7001
# dir="/root/project/assist-upgrade"
echo $1
cd $1 && npm install
...
# # 更新文件
# # git reset --hard
# gitPullOutput=$(expect -c "spawn git pull;expect \"*Username*\" { send \"292621171@qq.com\n\";exp_continue } \"*Password\" { send \"kai12345678\n\" };interact")
# echo '--------------'
# echo "$gitPullOutput"
# echo '--------------'
# echo "小助服务代码拉取成功!"
...
pid=$(lsof -i :7001| grep "node"| awk '{print $2}' | awk -F"/" '{ print $1 }')

expect -c "spawn kill -9 $pid && nohup npm run dev &"
# kill -9 $pid
# nohup npm run dev &

# 端口进程
# netstat -nlp tcp | grep :7002 | awk '{print $7}' | awk -F"/" '{ print $1 }'
# pid=$(netstat -nlp tcp | grep :$port | awk '{print $7}' | awk -F"/" '{ print $1 }')
# if [[ "$pid" != "" ]]; then
#   echo "项目运行中"
#   if [[ $isUpdatePackage || $isUpdatePackage2 ]]; then
#     echo "项目开始重启, 请稍后尝试访问"
